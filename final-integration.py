...
def start():
    global board
    board.digital[13].write(1) # Integration with hardware
...
def getFaceDirection(shape, size):
    image_points = np.array([
                                ...
                                shape[36],    # Right eye right cornez
...
while(True):
    ...
    gray = frame
    rects = detector(gray, 0) #Face detection
    if(len(rects)):
        ...
        if(avgEAR<close_thresh):
            ...
            if(yawn_countdown and flag>=frame_thresh_3):
                ...
                if(map_flag):
                    map_flag = 0
                    map_counter+=1
            elif(flag>=frame_thresh_2 and getFaceDirection(shape, size)<0):  #Head pose Estimation
        ...
        if(map_counter>=3):
            ...
            stop()
            webbrowser.open("https://www.google.com/maps/search/hotels+or+motels+near+me") #web browsing
        ...
        cv2.drawContours(gray, [leftEyeHull], -1, eyeContourColor, 2)  #contour drawing( around eyes)
        cv2.drawContours(gray, [rightEyeHull], -1, eyeContourColor, 2)
        writeEyes(leftEye, rightEye, frame)
    if(avgEAR>close_thresh):
        alert.stop() #alerting mechanism
